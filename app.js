...
(function () {
  ...
  function renderAuth() {
    ...
    $('#root').innerHTML =
      '<div class="gate">' +
        ...
        '<p class="foot">Pointed at <code>' + h(S.url || '?') + '</code>. ' +
          'If that is not your project, fix <code>assets/config.js</code>.</p>' +
      '</div>';
  ...
  function doAuth() {
    ...
    call.then(function (r) {
      ...
      if (r.error) {
        msg.innerHTML = '<div class="msg err">' + h(r.error.message) + '</div>';
        diagnose(msg);
        return;
      }
  ...
  function diagnose(box) {
    var probe = S.url + '/auth/v1/health';
    var note = document.createElement('div');
    note.className = 'msg';
    note.style.background = '#F4EFE6';
    note.innerHTML = 'Checking the connection\u2026';
    box.appendChild(note);

    fetch(probe, { headers: { apikey: S.key } }).then(function (res) {
      return res.text().then(function (body) { return { s: res.status, b: body }; });
    }).then(function (r) {
      var ok = r.s === 200 && /gotrue/i.test(r.b);
      note.innerHTML = ok
        ? '<b>The project is reachable.</b> So the address is right and the problem is ' +
          'the request itself. If it said invalid login, the account or password is wrong. ' +
          'If it said signups are disabled, turn them back on in Supabase under ' +
          'Authentication, Sign In / Providers, Email.'
        : '<b>The project did not answer properly.</b><br>Tried <code>' + h(probe) +
          '</code><br>Got HTTP ' + r.s + ': <code>' + h(r.b.slice(0, 160)) + '</code>' +
          '<br><br>Open that address in a browser tab. If it does not return JSON ' +
          'mentioning GoTrue, the host in <code>assets/config.js</code> is not your ' +
          'project. Copy it again from Project Settings, then Data API, then Project URL.';
    }).catch(function (e) {
      note.innerHTML = '<b>Could not reach the project at all.</b><br>Tried <code>' +
        h(probe) + '</code><br>' + h((e && e.message) || e) +
        '<br><br>Either the host is wrong, or the project is paused. Check the Supabase ' +
        'dashboard for a Restore button.';
    });
  }
